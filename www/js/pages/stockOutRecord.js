...
window.Pages['/stock-out-records'] = {
  _filters: {},
    ...
    return `
      <div class="page">
        ...
        <div class="footer-btn-fixed">
          <button class="btn-primary" onclick="Pages['/stock-out-records'].onExport()">导出以上出库记录</button>
        </div>
      </div>
    `;
  },
  ...
  renderRecords() {
    const orders = Store.stockOut.query(this._filters);
    if (orders.length === 0) {
      return UI.empty('暂无出库记录');
    }
